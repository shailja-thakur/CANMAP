...
trans_power = pd.DataFrame(np.load((direc + 'normalized_signals/' + '-'.join(['normalized', timestamp, ecus.get(ecu), channel]) + ".npy")))
...
if len(cans.index[cans['id'].isin(mapper['trans'])]) > 20000:
    can_indices = cans.index[cans['id'].isin(mapper['trans'])][0:19000]
else:
    can_indices = cans.index[cans['id'].isin(mapper['trans'])]

#if len(cans.index[cans['id'].isin(mapper['abs'])]) > 20000:
#    can_abs = cans.index[cans['id'].isin(mapper['abs'])][0:19000]
#else#:
#    can_abs = cans.index[cans['id'].isin(mapper['abs'])]
