...
def load_gdls_powertraces(train_dir, number,module, format='bin'):
...
def map_atvp_ID(labels):


    labels = pd.DataFrame(labels.values, columns=['id'])
    #print(labels.index[labels['id'].isin(engine)])
    labels.loc[labels.index[labels['id'] == 'atvp2'], 'id'] = 1
    labels.loc[labels.index[labels['id'] == 'atvp3'], 'id'] = 2
    labels.loc[labels.index[labels['id'] == 'atvp4'], 'id'] = 3
    return labels['id'].as_matrix()

def map_ECU_ID_visit2(labels):
    brake = [0,61441, 65215]
    trans = 61442
    engine = [61443,61444,65247]

    labels = pd.DataFrame(labels.values, columns=['id'])
    #print(labels.index[labels['id'].isin(engine)])
    labels.loc[labels.index[labels['id'].isin(engine)], 'id'] = 1
    labels.loc[labels.index[labels['id'] == trans], 'id'] = 2
    labels.loc[labels.index[labels['id'].isin(brake)], 'id'] = 3
    return labels['id'].as_matrix()
...
def load_gdls_powertraces_db(train_dir, filename, format='bin'):
    ...
    df = pd.read_csv(filename_signal)
    print(df.shape)
    traces = df.iloc[:,0:400].as_matrix()
    #traces = l2_norm(traces)
    
    all_labels = df.iloc[:, -1]
    # visit2
    labels = map_ECU_ID_visit2(all_labels)
    # visit3
    #labels = map_atvp_ID(all_labels)
    print(all_labels)
    #all_traces = df.as_matrix()
    return traces, labels[:, np.newaxis]
...
def read_data_sets_keras(train_dir, number, module, fake_data=False, one_hot=False,
                        dtype=dtypes.float64, reshape=True,
                        validation_size=5000, format='pkl', data='kamal'):
    """Set the images and labels."""
    num_training = 400
    ...
    if data == "kamal":
        ...
        all_traces, all_labels = load_gdls_powertraces(train_dir, number, module)
    else:
        print("Enter a valid data source to read traces from")
        sys.exit(0)
    ...
    return train_traces, train_labels, validation_traces, validation_labels, test_traces, test_labels
